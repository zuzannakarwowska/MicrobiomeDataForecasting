...
def ridge_linear_forest(X, y):
    
    param_grid={
        'base_estimator__alpha':[0.1, 0.5, 0.9, 2, 10, 100, 150, 1000, 5000, 100000],
    ...
    model = GridSearchCV(
        LinearForestRegressor(Ridge(random_state = 0), random_state=42),
...
for i in range(len(FILTERED_CLR_DF.columns)):
    ...
    model = ridge_linear_forest(X_train, y_train)

    prediction_train = model.predict(X_train)
    prediction_test  = model.predict(X_test)
    denom_name = FILTERED_COUNT_DF.iloc[:, i].name
    
    #filename = 'ridge_lag3_models/ridge-linear-forest_{}.sav'.format(denom_name)
...
CLR_SUPERVISED_DF[:-40].to_csv('results/otu_prediction/ridge_true_train_lag{}.csv'.format(lag))
CLR_SUPERVISED_DF[-40:].to_csv('results/otu_prediction/ridge_true_test_lag{}.csv'.format(lag))

PREDICTION_TRAIN.to_csv('results/otu_prediction/ridge_pred_train_lag{}.csv'.format(lag))
PREDICTION_TEST.to_csv('results/otu_prediction/ridge_pred_test_lag{}.csv'.format(lag))

COEFF_DF.to_csv('results/otu_prediction/ridge_coeff_lag{}.csv'.format(lag))
F_IMPORTANCE_DF.to_csv('results/otu_prediction/ridge_feature_importance_lag{}.csv'.format(lag))
